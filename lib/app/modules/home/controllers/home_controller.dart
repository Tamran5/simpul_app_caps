...
class HomeController extends GetxController {
  ...
  Future<void> _fetchNotifStatus() async {
    try {
      final token    = await _getToken();
      final response = await http
          .get(Uri.parse(ApiConfig.notifUnreadCount), headers: _authHeaders(token))
  ...
  Future<void> markAllNotifRead() async {
    try {
      final token = await _getToken();
      await http
          .post(Uri.parse(ApiConfig.notifMarkRead), headers: _authHeaders(token))
          .timeout(const Duration(seconds: 10));

      notifications.value = notifications.map((n) => {...n, 'is_read': true}).toList();
  ...
  void _parseWedding(Map<String, dynamic> data) {
    ...
    if (epoch > 0) {
      _updateCountdownFrom(DateTime.fromMillisecondsSinceEpoch(epoch * 1000));
    }
  }
  ...
  void _parseProgress(Map<String, dynamic> progress) {
    ...
    legalProgress.value  = ((progress['legal_percent'] ?? 0) as num).toDouble() / 100;
    taskProgress.value   = ((progress['task_percent']  ?? 0) as num).toDouble() / 100;
  }
  ...
  Future<void> submitPairRequest() async {
    ...
    try {
      final token    = await _getToken();
      final response = await http
          .post(Uri.parse(ApiConfig.pairRequest),
  ...
  Future<void> respondToPair(String action) async {
    ...
    try {
      final token = await _getToken();
      await http
          .post(Uri.parse(ApiConfig.pairRespond),
              headers: _authHeaders(token),
  ...
  Future<void> updateWeddingDate(DateTime picked) async {
    ...
    try {
      ...
      final response = await http
          .patch(Uri.parse(ApiConfig.weddingDate),
              headers: _authHeaders(token),
      ...
      if (response.statusCode == 200) {
        final body = jsonDecode(response.body) as Map<String, dynamic>;
        weddingDateStr.value = body['data']?['display'] ?? _formatDate(picked);
        Get.snackbar('Tanggal Disimpan ✓', 'Hitung mundur diperbarui.',
            backgroundColor: const Color(0xFF3D6B5F),
            colorText: Colors.white,
            snackPosition: SnackPosition.TOP);
      }
      // Jika non-200: diam saja, optimistic value sudah di-set
    } catch (_) {
      // FIX: tidak tampilkan "Offline Mode" — tanggal sudah ter-set lokal
    }
  }
  ...
  void copyMyCode() {
    ...
    Get.snackbar('Disalin ✓', 'Kode ${myUniqueCode.value} berhasil disalin.',
        snackPosition: SnackPosition.TOP,
        backgroundColor: const Color(0xFF3D6B5F),
        colorText: Colors.white,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('Hubungkan Akun Simpul',
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold,
                  color: Color(0xFF1A1A1A))),
          const SizedBox(height: 4),
          const Text('Bagikan kode unikmu, atau masukkan kode pasangan.',
              style: TextStyle(fontSize: 12, color: Color(0xFF8A8A8A))),
          const SizedBox(height: 20),
          const Text('KODE UNIKMU',
              style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold,
                  color: Color(0xFF8A8A8A), letterSpacing: 1.2)),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: const Color(0xFF3D6B5F), width: 1.5),
            ),
            child: Row(
              ...
              children: [
                Obx(() => Text(
                  ...
                  style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900,
                      letterSpacing: 6.0, color: Color(0xFF3D6B5F)),
                )),
                GestureDetector(
          ...
          TextField(
            ...
            style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold,
                letterSpacing: 4, color: Color(0xFF1A1A1A)),
            decoration: InputDecoration(
              ...
              enabledBorder: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(14),
                  borderSide:
                      const BorderSide(color: Color(0xFFF0F0F0), width: 1.5)),
              focusedBorder: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(14),
                  borderSide:
                      const BorderSide(color: Color(0xFF3D6B5F), width: 2)),
              filled: true,
          ...
          Obx(() => SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: controller.isPairActionLoading.value
                  ...
                  : const Text('Kirim Permintaan Terhubung',
          ...
          children: [
            ...
            Text(
              ...
              style: const TextStyle(
                  fontSize: 13, color: Color(0xFF8A8A8A), height: 1.5),
            ),
